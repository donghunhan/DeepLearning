...
batch_mask = np.random.choice(10000, batch_size) # 랜덤으로 100개 뽑음
x_batch2 = x_test[batch_mask] # x_test데이터 100개
t_answer = t_test[batch_mask] # 실제 타겟값 100개

y = network.predict(x_batch2) # x에 대해 예측
y = np.argmax(y, axis=1) # 그중 가장 크게 나온 값의 인덱스
t_answer = np.argmax(t_answer, axis=1) # 실제 타겟값의 가장 큰 인덱스(one-hot인코딩이므로)

wrong_answers = y != t_answer # 틀린 지점들의 인덱스 가져옴
wrong_imgs = x_batch2[wrong_answers] # 틀린지점의 이미지 뽑아내기
